...
public class RippleIntroView extends RelativeLayout implements Runnable {
    private int mMaxRadius = 250;
    private int mInterval = 220;
    ...
    public void setValue(int interval,int delay){
    ...
    public void setPaintColor(){
        mRipplePaint.setColor(getResources().getColor(R.color.bluedark));
    }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int step = count; step <= mMaxRadius; step += mInterval) {
            //step越大越靠外就越透明
            mRipplePaint.setAlpha(70 * (mMaxRadius - step) / mMaxRadius);
