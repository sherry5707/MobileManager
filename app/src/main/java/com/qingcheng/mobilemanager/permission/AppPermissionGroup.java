...
import com.qingcheng.mobilemanager.utils.LocationUtil;
...
/*            if (packageInfo.applicationInfo.targetSdkVersion <= Build.VERSION_CODES.LOLLIPOP_MR1
            ...
            LogUtil.d("getPermissionFlags="+flags);
   ...
   public boolean isReviewRequired() {
    ...
    public boolean areRuntimePermissionsGranted(String[] filterPermissions) {
        if (LocationUtil.isLocationGroupAndProvider(mName, mPackageInfo.packageName)) {
            return LocationUtil.isLocationEnabled(mContext);
        }
        final int permissionCount = mPermissions.size();
        for (int i = 0; i < permissionCount; i++) {
            Permission permission = mPermissions.valueAt(i);
