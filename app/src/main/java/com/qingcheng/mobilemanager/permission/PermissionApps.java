...
public class PermissionApps {
    ...
    private PermissionGroup mGroup;

    public PermissionApps(Context context, PermissionGroup group, Callback callback) {
        this(context, group, callback, null);
    }

    public PermissionApps(Context context, PermissionGroup group, Callback callback, PmCache cache) {
        mCache = cache;
        mContext = context;
    ...
    public int getGrantedCount(ArraySet<String> launcherPkgs) {
        ...
        for (PermissionApp app : mPermApps) {
            if (!PermissionUtil.shouldShowPermission(app)) {
                continue;
            }
            if (PermissionUtil.isSystem(app, launcherPkgs)) {
    ...
    private List<PermissionApp> loadPermissionApps() {
        PackageItemInfo groupInfo = getGroupInfo(mGroup.getName());
        if (groupInfo == null) {
            return Collections.emptyList();
        }

        List<PermissionInfo> groupPermInfos = getGroupPermissionInfos(mGroup.getName());
    ...
    private void loadGroupInfo() {
        ...
        try {
            info = mPm.getPermissionGroupInfo(mGroup.getName(), 0);
        } catch (NameNotFoundException e) {
            try {
                PermissionInfo permInfo = mPm.getPermissionInfo(mGroup.getName(), 0);
                if (permInfo.protectionLevel != PermissionInfo.PROTECTION_DANGEROUS) {
                    Log.w(LOG_TAG, mGroup.getName() + " is not a runtime permission");
                    return;
                }
                info = permInfo;
            } catch (NameNotFoundException reallyNotFound) {
                Log.w(LOG_TAG, "Can't find permission: " + mGroup.getName(), reallyNotFound);
                return;
            }
        }
        ...
        mIcon = PermissionUtil.applyTint(mContext, mIcon, android.R.attr.colorControlNormal);
    }
    ...
    private class PermissionAppsLoader extends AsyncTask<Void, Void, List<PermissionApp>> {
        ...
        protected void onPostExecute(List<PermissionApp> result) {
            ...
            createMap(result);
            if (mCallback != null) {
                mCallback.onPermissionsLoaded(PermissionApps.this,mGroup);
            }
        }
    }
    ...
    public interface Callback {
        void onPermissionsLoaded(PermissionApps permissionApps,PermissionGroup group);
    }
}
